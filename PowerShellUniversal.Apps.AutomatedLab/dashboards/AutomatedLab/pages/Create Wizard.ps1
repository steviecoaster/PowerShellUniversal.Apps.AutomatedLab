$WizardPage = New-UDPage -Url '/Create-Lab' -Name 'Create A Lab' -Content {
    ...
    $Session:BuiltInRoleParameters = (Get-LabConfigurationItem -Name 'ValidationSettings')
            ...
            if ((-not $Session:OSLoaded -and -not $Session:OSLoadError) -or -not $Session:AdaptersLoaded) {
                ...
                New-UDStepper -Id 'LabBuilderStepper' -OnFinish {
                    ...
                    New-UDStep -OnLoad {
                        ...
                        # Built-In Roles Assignment Card
                        New-UDCard -Title 'Built-In Roles (Optional)' -Content {
                            New-UDGrid -Container -Content {
                                ...
                                New-UDGrid -Item -ExtraSmallSize 12 -SmallSize 8 -Content {
                                    New-UDDynamic -Id 'BuiltInRoleSelect' -Content {
                                        try {
                                            $availableBuiltInRoles = [Enum]::GetValues([AutomatedLab.Roles])
                                            $availableBuiltInRolesFiltered = $availableBuiltInRoles |
                                            Where-Object { $_.ToString() -notin 'ADDS', 'SQLServer', 'SharePoint', 'SCVMM', 'SCOM', 'Dynamics', 'RDS' }
                                            if ($availableBuiltInRolesFiltered -and $availableBuiltInRolesFiltered.Count -gt 0) {
                                                New-UDSelect -Id 'BuiltInVMRole' -Label 'Available Built-In Roles' -FullWidth -Option {
                                                    foreach ($role in $availableBuiltInRolesFiltered) {
                                                        New-UDSelectOption -Name $role -Value $role
                                                    }
                                                } -OnChange {
                                                    # Enable the Add Role Parameters button when a role is selected
                                                    $selectedRole = $EventData
                                                    if ($selectedRole) {
                                                        Set-UDElement -Id 'AddRoleParametersButton' -Properties @{ disabled = $false }
                                                    } else {
                                                        Set-UDElement -Id 'AddRoleParametersButton' -Properties @{ disabled = $true }
                                                    }
                                                }
                                            } else {
                                                New-UDSelect -Id 'BuiltInVMRole' -Label 'Available Built-In Roles' -FullWidth -Option {
                                                    New-UDSelectOption -Name 'No built-in roles available' -Value '' -Disabled
                                                }
                                            }
                                        } catch {
                                            New-UDSelect -Id 'BuiltInVMRole' -Label 'Available Built-In Roles' -FullWidth -Option {
                                ...
                                New-UDGrid -Item -ExtraSmallSize 12 -SmallSize 4 -Content {
                                    New-UDStack -Direction "column" -Content {
                                        New-UDButton -Text "Add Role Parameters" -Id 'AddRoleParametersButton' -Color secondary -FullWidth -OnClick {
                                            ...
                                            if (($ValidProperties -and $ValidProperties.Count -gt 0) -or ($MandatoryParams -and $MandatoryParams.Count -gt 0)) {

                                                # Check if this role already exists in CurrentBuiltInVMRoles to prepopulate values
                                                $existingRole = $null
                                                if ($Session:CurrentBuiltInVMRoles) {
                                                    $existingRole = $Session:CurrentBuiltInVMRoles | Where-Object { $_.RoleName -eq $selectedRole }
                                                }

                                                # Don't clear parameter values if we're reopening the modal for the same role
                                                # This preserves values that were entered but not yet saved to CurrentBuiltInVMRoles
                                                if (-not $Session:BuiltInRoleOptionalParameterValues) {
                                                    $Session:BuiltInRoleOptionalParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                }
                                                if (-not $Session:BuiltInRoleMandatoryParameterValues) {
                                                    $Session:BuiltInRoleMandatoryParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                }

                                                Show-UDModal -Content {
                                                    New-UDCard -Title "Parameters for Role: $selectedRole" -Content {
                                                        New-UDGrid -Container -Content {
                                                            foreach ($manparam in $MandatoryParams) {
                                                                # Get existing value from either saved role or current session
                                                                $existingValue = ''
                                                                # First check if value exists in current session (recently entered but not saved)
                                                                if ($Session:BuiltInRoleMandatoryParameterValues) {
                                                                    $sessionParam = $Session:BuiltInRoleMandatoryParameterValues | Where-Object { $_.Name -eq $manparam }
                                                                    if ($sessionParam) {
                                                                        $existingValue = $sessionParam.Value
                                                                    }
                                                                }
                                                                # If not in session, check saved role
                                                                if ([string]::IsNullOrEmpty($existingValue) -and $existingRole -and $existingRole.MandatoryParameters) {
                                                                    $existingParam = $existingRole.MandatoryParameters | Where-Object { $_.Name -eq $manparam }
                                                                    if ($existingParam) {
                                                                        $existingValue = $existingParam.Value
                                                                    }
                                                                }
                                                                New-UDGrid -Item -ExtraSmallSize 12 -SmallSize 6 -Content {
                                                                    New-UDTextbox -Id "ManParam_$manparam" -Label "$manparam *" -Placeholder "Enter value for $manparam" -FullWidth -Value $existingValue
                                                                } -Style @{ 'margin-bottom' = '16px'; 'padding' = '0 8px' }
                                                            }
                                                            foreach ($param in $ValidProperties) {
                                                                # Get existing value from either saved role or current session
                                                                $existingValue = ''
                                                                # First check if value exists in current session (recently entered but not saved)
                                                                if ($Session:BuiltInRoleOptionalParameterValues) {
                                                                    $sessionParam = $Session:BuiltInRoleOptionalParameterValues | Where-Object { $_.Name -eq $param }
                                                                    if ($sessionParam) {
                                                                        $existingValue = $sessionParam.Value
                                                                    }
                                                                }
                                                                # If not in session, check saved role
                                                                if ([string]::IsNullOrEmpty($existingValue) -and $existingRole -and $existingRole.OptionalParameters) {
                                                                    $existingParam = $existingRole.OptionalParameters | Where-Object { $_.Name -eq $param }
                                                                    if ($existingParam) {
                                                                        $existingValue = $existingParam.Value
                                                                    }
                                                                }
                                                    ...
                                                    New-UDButton -Text "Save & Close" -Color default -OnClick {
                                                        ...
                                                        foreach ($param in $ValidProperties) {
                                                            $value = (Get-UDElement -Id "Param_$param").value
                                                            ...
                                                            $Session:BuiltInRoleOptionalParameterValues.Add($row)
                                                        }
                                                        foreach ($manparam in $MandatoryParams) {
                                                            $value = (Get-UDElement -Id "ManParam_$manparam").value

                                                            if ([string]::IsNullOrEmpty($value)) {
                                                                Show-UDToast -Message "Parameter '$manparam' is mandatory and must be filled out."
                                                                return
                                                            } else {
                                                                $row = [pscustomobject]@{
                                                                    Name  = $manparam
                                                                ...
                                                                $Session:BuiltInRoleMandatoryParameterValues.Add($row)
                                                            }
                                                        }
                                                        Hide-UDModal
                                        ...
                                        New-UDButton -Text "Add Role" -Color secondary -FullWidth -OnClick {
                                            ...
                                            if ($selectedRole) {
                                                ...
                                                if (-not $Session:CurrentBuiltInVMRoles) {
                                                    $Session:CurrentBuiltInVMRoles = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                }
                                                if ($Session:CurrentBuiltInVMRoles -and $Session:CurrentBuiltInVMRoles.Count -gt 0) {
                                                    # Check if role is already added - if so, remove the old one to replace it
                                                    $existingRoleIndex = -1
                                                    for ($i = 0; $i -lt $Session:CurrentBuiltInVMRoles.Count; $i++) {
                                                        if ($Session:CurrentBuiltInVMRoles[$i].RoleName -eq $selectedRole) {
                                                            $existingRoleIndex = $i
                                                            break
                                                        }
                                                    }
                                                    if ($existingRoleIndex -ge 0) {
                                                        $Session:CurrentBuiltInVMRoles.RemoveAt($existingRoleIndex)
                                                        Show-UDToast -Message "Updated existing role '$selectedRole'"
                                                    }
                                                }
                                                $roleEntry = [PSCustomObject]@{
                                                    RoleName            = $selectedRole
                                                    OptionalParameters  = @($Session:BuiltInRoleOptionalParameterValues.ToArray())
                                                    MandatoryParameters = @($Session:BuiltInRoleMandatoryParameterValues.ToArray())
                                                }
                                                $Session:CurrentBuiltInVMRoles.Add($roleEntry)
                                                # Clear parameter values for next role addition
                                                $Session:BuiltInRoleOptionalParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                $Session:BuiltInRoleMandatoryParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                Set-UDElement -Id 'BuiltInVMRole' -Properties @{ value = '' }
                                                Set-UDElement -Id 'AddRoleParametersButton' -Properties @{ disabled = $true }
                                                        ...
                                                        if ($EventData.MandatoryParameters -and $EventData.MandatoryParameters.Count -gt 0) {
                                                            ...
                                                            New-UDStack -Direction "column" -Content {
                                                                ...
                                                                ForEach-Object {
                                                                    ...
                                                                    New-UDTypography -Text $s -Style @{ 'white-space' = 'pre-wrap' }
                                                                }
                                                            }
                                                        } else {
                                                            $MandatoryParameterCount = $EventData.MandatoryParameters.Count
                                                    ...
                                                    New-UDTableColumn -Property Actions -Title "Actions" -Render {
                                                        New-UDStack -Direction "row" -Spacing 1 -Content {
                                                            New-UDButton -Text "Edit" -Size "small" -Color "primary" -OnClick {
                                                                ...
                                                                # Get the current role data to prepopulate the modal
                                                                $existingRole = $Session:CurrentBuiltInVMRoles | Where-Object { $_.RoleName -eq $selectedRole }

                                                                # Clear and populate session variables with current role values
                                                                $Session:BuiltInRoleOptionalParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                                $Session:BuiltInRoleMandatoryParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'

                                                                # Pre-populate session variables with existing values for the modal
                                                                if ($existingRole.OptionalParameters) {
                                                                    foreach ($param in $existingRole.OptionalParameters) {
                                                                        $Session:BuiltInRoleOptionalParameterValues.Add($param)
                                                                    }
                                                                }
                                                                if ($existingRole.MandatoryParameters) {
                                                                    foreach ($param in $existingRole.MandatoryParameters) {
                                                                        $Session:BuiltInRoleMandatoryParameterValues.Add($param)
                                                                    }
                                                                }

                                                                if (($ValidProperties -and $ValidProperties.Count -gt 0) -or ($MandatoryParams -and $MandatoryParams.Count -gt 0)) {
                                                                    Show-UDModal -Content {
                                                                        New-UDCard -Title "Edit Parameters for Role: $selectedRole" -Content {
                                                                            New-UDGrid -Container -Content {
                                                                                foreach ($manparam in $MandatoryParams) {
                                                                                    # Get existing value from current session
                                                                                    $existingValue = ''
                                                                                    if ($Session:BuiltInRoleMandatoryParameterValues) {
                                                                                        $sessionParam = $Session:BuiltInRoleMandatoryParameterValues | Where-Object { $_.Name -eq $manparam }
                                                                                        if ($sessionParam) {
                                                                        ...
                                                                        New-UDButton -Text "Save & Close" -Color default -OnClick {
                                                                            ...
                                                                            # Update the existing role in CurrentBuiltInVMRoles
                                                                            $roleIndex = -1
                                                                            for ($i = 0; $i -lt $Session:CurrentBuiltInVMRoles.Count; $i++) {
                                                                                if ($Session:CurrentBuiltInVMRoles[$i].RoleName -eq $selectedRole) {
                                                                                    $roleIndex = $i
                                                                            ...
                                                                            if ($roleIndex -ge 0) {
                                                                                $Session:CurrentBuiltInVMRoles[$roleIndex].OptionalParameters = @($Session:BuiltInRoleOptionalParameterValues.ToArray())
                                                                                $Session:CurrentBuiltInVMRoles[$roleIndex].MandatoryParameters = @($Session:BuiltInRoleMandatoryParameterValues.ToArray())
                                                                            }

                                                                            # Clear parameter values after updating
                                                                            $Session:BuiltInRoleOptionalParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                                                            $Session:BuiltInRoleMandatoryParameterValues = New-Object -TypeName 'System.Collections.Generic.List[System.Object]'
                                    ...
                                    New-UDButton -Text "Add Virtual Machine to Lab" -Color "primary" -Size "large" -FullWidth -OnClick {
                                        ...
                                        if ($vmName -and $vmSize -and $vmOS) {
                                            ...
                                            $newVM = [PSCustomObject]@{
                                                ...
                                                BuiltInRoles    = if ($Session:CurrentBuiltInVMRoles) { $Session:CurrentBuiltInVMRoles } else { @() }
                                                ...
                                                New-UDStack -Direction "column" -Content {
                                                    ...
                                                    foreach ($role in $EventData.BuiltInRoles) {
                                                        if (($role.MandatoryParameters -and $role.MandatoryParameters.Count -gt 0) -or ($role.OptionalParameters -and $role.OptionalParameters.Count -gt 0)) {
                                                            ...
                                                            # Show optional parameters (only those with values)
